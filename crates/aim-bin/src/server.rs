...
use aim_core::message::{InteractiveUi, UiKind};
...
impl Server {
    ...
    async fn handle_monitor_event(&self, event: MonitorEvent) -> Result<()> {
        match event {
            MonitorEvent::NewMessages(messages) => {
                ...
                let mut by_session: HashMap<String, Vec<&aim_core::message::NewMessage>> = HashMap::new();
                for msg in &messages {
                    tracing::debug!(
                    ...
                    if msg.role != "assistant"
                        && msg.content_type != aim_core::message::ContentType::ToolResult
                    {
                        continue;
                    }
                    if !msg.is_complete {
                    ...
                    if msg.content_type == aim_core::message::ContentType::Thinking {
                        continue;
                    }
                ...
                for (_sid, group) in &by_session {
                    ...
                    let has_text = group.iter().any(|m| m.content_type == aim_core::message::ContentType::Text);
                    ...
                    for msg in group {
                        ...
                        use aim_core::message::ContentType;
                        match msg.content_type {
                            ...
                            _ => {
                                ...
                                let converted = aim_parser::table::convert_tables(&msg.text);
                                if !merged.is_empty() {
                                    merged.push('\n');
                                }
    ...
    async fn unbound_windows(&self, state: &aim_core::session::ServerState) -> Vec<browser::WindowEntry> {
    ...
    async fn probe_interactive_uis(&self) -> Result<()> {
        ...
        for binding in &state.thread_bindings {
            ...
            let clean = aim_parser::terminal::TerminalParser::strip_ansi(&pane_text);
            let agent = aim_parser::terminal::TerminalParser::detect_agent(&clean, "claude");
            let ui = aim_parser::terminal::TerminalParser::detect_interactive(&clean, agent);
...
async fn transcribe_voice(api_key: &str, base_url: &str, audio_data: &[u8]) -> Result<String> {
    ...
    let part = reqwest::multipart::Part::bytes(audio_data.to_vec())
        .file_name("voice.ogg")
        .mime_str("audio/ogg")
        .map_err(|e| aim_core::error::Error::Config(format!("mime error: {e}")))?;
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| aim_core::error::Error::Telegram(format!("transcription request failed: {e}")))?;

    if !resp.status().is_success() {
        let status = resp.status();
        let body = resp.text().await.unwrap_or_default();
        return Err(aim_core::error::Error::Telegram(format!(
    ...
    let json: serde_json::Value = resp
        .json()
        .await
        .map_err(|e| aim_core::error::Error::Telegram(format!("transcription JSON parse: {e}")))?;
