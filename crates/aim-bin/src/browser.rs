...
mod tests {
    ...
    fn test_read_dir_entries_filters_hidden() {
        let dir = std::env::temp_dir().join("aim-test-browser");
    ...
    fn test_read_dir_entries_sorted() {
        let dir = std::env::temp_dir().join("aim-test-browser-sorted");
        let _ = std::fs::create_dir_all(&dir);
    ...
    fn test_window_picker_mode() {
        ...
        rt.block_on(async {
            ...
            let windows = vec![
                WindowEntry { window_id: "@1".into(), name: "aim-400".into(), current_command: "claude".into() },
