...
use aim_core::message::{NewMessage, SessionId};
...
impl SessionMonitor {
    ...
    ///
    /// * `aim_dir` — the `~/.aim` directory (contains session_map.json)
    ...
    pub fn new(
        ...
        Self {
            session_map_path: aim_dir.join("session_map.json"),
    ...
    async fn check_session_map(&mut self, tx: &mpsc::UnboundedSender<MonitorEvent>) -> Result<()> {
        ...
        let map: HashMap<String, String> = serde_json::from_str(&data)
            .map_err(|e| aim_core::error::Error::Parse(format!("invalid session map: {e}")))?;
    ...
    async fn poll_sessions(&self, tx: &mpsc::UnboundedSender<MonitorEvent>) -> Result<()> {
        ...
        for session_id in &session_ids {
            ...
            let (entries, file_size) =
                aim_parser::jsonl::JsonlParser::read_new(&path, offset).await?;
