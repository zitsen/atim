...
use aim_core::message::{Button, ImEvent, ImEventKind, MessageId, MessageTarget};
    ...
    for event in parser {
        match event {
            Event::Start(tag) => match tag {
                ...
                Tag::CodeBlock(CodeBlockKind::Fenced { .. }) => out.push_str("<pre>"),
                Tag::CodeBlock(CodeBlockKind::Indented) => out.push_str("<pre>"),
                _ => {}
            },
...
impl TelegramAdapter {
    ...
    async fn get_updates(&self, offset: Option<i64>) -> Result<Vec<serde_json::Value>> {
        ...
        // Long-poll needs a timeout > the server-side `timeout` parameter
        // so the client doesn't race the server response.
        let result = self
            .api_post_timeout("getUpdates", &params, Duration::from_secs(65))
            .await?;
...
impl ImAdapter for TelegramAdapter {
    ...
    async fn send_photo(&self, target: &MessageTarget, filename: &str, data: &[u8]) -> Result<MessageId> {
        ...
        let mut req = self.client.post(&url).multipart(
        ...
        if let Some(thread) = target.thread_id {
            req = req.multipart(
                reqwest::multipart::Form::new()
                    .text("message_thread_id", thread.0.to_string()),
            );
        }
...
fn parse_message(msg: &serde_json::Value) -> Option<ImEvent> {
    ...
    let user_id = msg["from"]["id"].as_i64()?;
    ...
    let target = MessageTarget {
        chat_id: aim_core::message::ChatId(chat_id),
        thread_id: thread_id.map(aim_core::message::ThreadId),
    };
    ...
    Some(ImEvent {
        user_id: aim_core::message::UserId(user_id),
        target,
        kind,
    })
}
